...
async def run_test():
    ...
    try:
        ...
        # -> Input the 6-digit verification code and click 'Fazer Login' to complete login as admin user.
        frame = context.pages[-1]
        # Input 6-digit verification code for admin login
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div/div[2]/form/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        

        # -> Click 'Fazer Login' button to complete login as admin user.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Organization creation failed due to transactional error').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Admin operations for creating, reading, updating, and deleting organizations did not complete successfully with transactional and idempotent guarantees.")
