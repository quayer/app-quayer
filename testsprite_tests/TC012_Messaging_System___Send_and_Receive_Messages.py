...
async def run_test():
    ...
    try:
        ...
        # -> Input email and click 'Continuar com Email' to log in as user with chat session access
        ...
        # -> Open an active chat session
        frame = context.pages[-1]
        # Click on the first available active chat session or relevant element to open chat
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the 6-digit verification code to complete login and proceed to open active chat session
        frame = context.pages[-1]
        # Input 6-digit verification code sent to email
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div/div[2]/form/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        

        # -> Click 'Fazer Login' button to complete login and proceed to open active chat session
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test plan failed: Sending, receiving, and managing messages within active chat sessions did not complete successfully, including message media handling and template usage.")
