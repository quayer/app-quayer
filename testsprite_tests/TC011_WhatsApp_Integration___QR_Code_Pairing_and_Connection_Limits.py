...
async def run_test():
    ...
    try:
        ...
        # -> Input email and click 'Continuar com Email' to log in as user with WhatsApp integration access
        ...
        # -> Input the 6-digit verification code to complete login and access WhatsApp connection dashboard
        frame = context.pages[-1]
        ...
        # -> Click the 'Fazer Login' button to complete login and navigate to WhatsApp connection dashboard
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=QR Code Pairing Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: QR code pairing initiation did not complete successfully. The test plan requires verifying QR code generation with countdown, step-by-step instructions, and enforcement of connection limits per organization.")
