...
async def run_test():
    ...
    try:
        ...
        # -> Click 'Fazer Login' button to complete login and proceed to subscribe to SSE endpoint
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Real-time SSE Event Stream Active').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: SSE subscriptions to real-time event streams, connection stability, event ordering, and error handling could not be verified as the test plan execution failed.")
