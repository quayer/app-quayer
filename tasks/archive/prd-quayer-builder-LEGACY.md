...
> **Versão:** 1.2 (refs atualizadas em 2026-05-10 — `specs/home-chat-builder/` e `quayer-ds-v3.html` não existem mais; trabalho já em execução)
> **Data:** 2026-04-08 (original), 2026-05-10 (refs corrigidas)
> **Status:** In progress — backend `src/server/ai-module/builder/` ativo (projects, chat, deploy, sub-agents); frontend `src/client/components/projetos/` (44 arquivos)
> **Fontes vivas:**
> - `src/server/ai-module/builder/**` — implementação real (projects 17 actions + chat 3 + deploy 3); skill `.claude/skills/quayer-builder.md`
> - `src/app/globals.css` — tokens DS v3 globais (cores amber, bg `#000`, DM Sans, radius, spacing, shadows) — migrados de `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) (deletado)
> - `src/client/components/ds/logo.tsx` — logo Q-bolt + wordmark (componente vivo, não HTML estático)
> - `src/app/(auth)/**` — **páginas de referência** (DS v3 aplicado em login/signup/verify/google-callback via `isAuthV3Enabled` + `NEXT_PUBLIC_AUTH_V3`) — toda página refatorada DEVE seguir esse padrão visual
...
- **Tokens do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo)** (cores amber, bg preto #000, DM Sans, radius, spacing, shadows)
- **Logo nova** do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) (Q-bolt + wordmark Quayer em DM Sans black)
...
9. **Logo nova em toda navegação:** substituir a logo antiga pelo Q-bolt + wordmark do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) em todos os headers, sidebars, loading states
...
**Acceptance Criteria:**
- [ ] Ler `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) e identificar todos os CSS custom properties
...
> **Princípio:** cada story DEVE usar `(auth)/*` existente como referência visual. Ou seja: bg `var(--color-bg)` preto, text `var(--color-fg)` branco, fonts DM Sans/Mono, amber accents, tokens de radius/shadow, **logo nova do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo)**.
>
...
- [ ] Logo Q-bolt + wordmark "Quayer" do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) no header principal
...
- **FR-23:** O sistema DEVE usar a **logo nova do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo)** (Q-bolt + wordmark Quayer em DM Sans black) em todos os headers, sidebars, loading states, e empty states das páginas refatoradas. Nenhuma ocorrência da logo antiga deve permanecer nas páginas refatoradas.
...
**Tokens a preservar (fonte: `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo)):**
...
- `src/server/ai-module/builder/**` — implementação backend (skill `.claude/skills/quayer-builder.md` resume)
- `src/client/components/projetos/**` — implementação frontend (44 arquivos: chat + preview tabs + cards)
- `src/app/globals.css` — tokens DS v3 globais (substitui o antigo `quayer-ds-v3.html`)
- `src/client/components/ds/logo.tsx` — logo Q-bolt + wordmark (componente, substitui referências ao HTML)
...
- [x] **Logo nova do `src/app/globals.css` (tokens) + `src/client/components/ds/logo.tsx` (logo) obrigatória em toda página refatorada**
