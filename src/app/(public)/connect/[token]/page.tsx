...
export default function ConnectPage() {
  ...
  const validateToken = async () => {
    try {
      const res = await fetch(`/api/v1/share/validate/${token}`)
      ...
      setInstanceName(json.data.instanceName || 'WhatsApp')
      setStep('loading')
  ...
  const generateQRCode = async () => {
    try {
      const res = await fetch('/api/v1/share/qr', {
        method: 'POST',
      ...
      if (!res.ok || !json.data) {
        setStep('error')
        setErrorMessage(json.message || 'Erro ao gerar QR Code')
        return
      }

      setQrCode(json.data.qrCode)
      setTimeLeft(Math.floor((json.data.expires || 120000) / 1000))
      setStep('qr')
    } catch (error: unknown) {
      setStep('error')
  ...
  const checkConnectionStatus = async () => {
    try {
      const res = await fetch(`/api/v1/share/status/${token}`)
      const json = await res.json()
