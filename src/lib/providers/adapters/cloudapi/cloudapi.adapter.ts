...
/**
 * Ciphertext shape produced by lib/crypto.encrypt(): `${ivHex}:${dataHex}`.
 * Both halves are hex; the IV is 16 bytes => 32 hex chars.
 */
const CIPHERTEXT_RE = /^[0-9a-f]{32}:[0-9a-f]+$/i;

/**
 * Decrypt a Cloud API secret stored on the Connection row.
 *
 * Per channel-credentials.contract.ts (SECRET_FIELDS), the route encrypts
 * `cloudApiAccessToken` (and `cloudApiVerifyToken`) with lib/crypto before
 * persisting. This adapter therefore decrypts before use.
 *
 * Defensive/backward-compatible: legacy rows from the old create-instance
 * flow stored the token in plaintext. If the value is not in the
 * `iv:data` ciphertext format (or decrypt fails), it is returned as-is so
 * existing connections keep working.
 */
function decryptSecret(value: string): string {
  if (!CIPHERTEXT_RE.test(value)) {
    // Not our ciphertext format — assume legacy plaintext token.
    return value;
  }
  try {
    return decrypt(value);
  } catch (error) {
    console.error('[CloudAPI] Failed to decrypt stored secret, using raw value:', error);
    return value;
  }
}
...
export class CloudAPIAdapter implements IWhatsAppProvider {
  ...
  async sendInteractiveList(instanceId: string, data: SendInteractiveListInput): Promise<MessageResult> {
    ...
    const response = await client.sendInteractiveList(data.to, {
  ...
  async sendInteractiveButtons(instanceId: string, data: SendInteractiveButtonsInput): Promise<MessageResult> {
    ...
    const response = await client.sendInteractiveButtons(data.to, {
      header: data.header ? {
  ...
  private async getClientForInstance(instanceId: string): Promise<CloudAPIClient> {
    ...
    // Fetch credentials from database (Connection columns per
    // channel-credentials.contract.ts: cloudApi* fields).
    ...
    // The access token is a SECRET encrypted by the route before persist
    // (SECRET_FIELDS in the contract). Decrypt before handing it to the
    // Graph API client so the Bearer header carries the real token.
    const client = new CloudAPIClient({
      accessToken: decryptSecret(connection.cloudApiAccessToken),
