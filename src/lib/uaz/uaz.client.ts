...
export class UAZClient {
  ...
  private async request<T = any>(
    ...
    const headers: Record<string, string> = {
      ...
      token: token || this.config.adminToken,
    };
  ...
  /**
   * 10. Enviar menu interativo (botões)
   */
  async sendMenu(
    instanceId: string,
    params: {
    ...
    return this.request('POST', `/instance/${instanceId}/send/menu`, params, token);
  }

  /**
   * 11. Enviar mensagem com mídia
   */
  async sendMedia(
    instanceId: string,
    params: {
