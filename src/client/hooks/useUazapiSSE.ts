...
export interface UseUazapiSSEOptions {
  ...
  /** Initial delay para reconexão em ms */
...
export function useUazapiSSE(options: UseUazapiSSEOptions) {
  ...
  const retryDelayRef = useRef(reconnectDelay)
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      eventSource.onopen = () => {
        ...
        retryDelayRef.current = reconnectDelay
      ...
      eventSource.onerror = (e) => {
        ...
        // Auto-reconectar with exponential backoff
        if (autoReconnect && enabled) {
          setReconnectAttempts((prev) => {
            const nextAttempt = prev + 1
            if (nextAttempt > maxRetries) {
              setSseDisconnected(true)
              console.error(`[UAZapi SSE] Max retries (${maxRetries}) exceeded. Giving up.`)
              return nextAttempt
            }

            const delay = retryDelayRef.current
            console.log(`[UAZapi SSE] Reconnecting in ${delay / 1000}s (attempt ${nextAttempt}/${maxRetries})`)
