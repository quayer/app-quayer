...
export function useInstanceSSE(options: UseInstanceSSEOptions) {
      ...
      if (autoInvalidate) {
        ...
        switch (eventType) {
          case 'instance.status':
            // Debounce por instância — keys devem bater com useInstances/useInstance
  ...
  // Function to create and connect an EventSource with exponential backoff
  const connectSSE = useCallback(() => {
    ...
    try {
      ...
      eventSource.onerror = (e) => {
        ...
        console.log(`[SSE] Reconnecting in ${delay / 1000}s (attempt ${retryCountRef.current}/${MAX_RETRIES})`)
  ...
  }, [getSSEUrl, enabled, handleSSEEvent])
