...
// LEGACY: removed after Builder IA pivot — was: import { useCreateInstance } from '@/hooks/useInstance'
// TODO: re-wire when WhatsApp instance controller is restored
function useCreateInstance() {
  return {
    mutateAsync: async (_payload: any): Promise<any> => {
      throw new Error('WhatsApp instance controller not available')
    },
  }
}

// LEGACY: removed after Builder IA pivot — was: import { usePermissions } from '@/hooks/usePermissions'
function usePermissions() {
  return { canSelectBroker: false }
}

// LEGACY: removed after Builder IA pivot — was: import type { CreateInstanceInput } from '@/features/instances/instances.interfaces'
...
// LEGACY: removed after Builder IA pivot — was: import { BrokerType } from '@/features/instances/instances.interfaces'
const BrokerType = {
  UAZAPI: 'UAZAPI',
  CLOUDAPI: 'CLOUDAPI',
} as const
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              {canSelectBroker && (
                <div className="space-y-3">
                  <Label>Tipo de Broker / Provedor</Label>
              ...
              {isCloudAPI && (
                <>
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="cloudApiAccessToken">Token de Acesso *</Label>
                    <Input
                      id="cloudApiAccessToken"
                      type="password"
                      placeholder="EAAxxxxxxx..."
                      disabled={isSubmitting}
                      {...form.register('cloudApiAccessToken')}
                    />
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="cloudApiPhoneNumberId">ID do Telefone (Phone Number ID) *</Label>
                    <Input
                      id="cloudApiPhoneNumberId"
                      placeholder="123456789012345"
                      disabled={isSubmitting}
                      {...form.register('cloudApiPhoneNumberId')}
                    />
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="cloudApiWabaId">ID da WABA (WhatsApp Business Account) *</Label>
                    <Input
                      id="cloudApiWabaId"
                      placeholder="987654321098765"
                      disabled={isSubmitting}
                      {...form.register('cloudApiWabaId')}
                    />
                    <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="phoneNumber">Numero do Telefone (Opcional)</Label>
                <Input
                  id="phoneNumber"
