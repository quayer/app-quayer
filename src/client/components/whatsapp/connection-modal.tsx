...
// LEGACY: removed after Builder IA pivot — was: import type { QRCodeResponse } from '@/features/instances/instances.interfaces'
type QRCodeResponse = {
  qrcode: string
  expires: number
}

// LEGACY: removed after Builder IA pivot — was: import { useConnectInstance, useInstanceStatus } from '@/hooks/useInstance'
// TODO: re-wire when WhatsApp instance controller is restored
function useConnectInstance() {
  return {
    isPending: false,
    mutateAsync: async (_instanceId: string): Promise<QRCodeResponse> => {
      throw new Error('WhatsApp instance controller not available')
    },
  }
}
function useInstanceStatus(_instanceId: string, _enabled: boolean) {
  return { data: null as { status?: string } | null, isLoading: false }
}
...
export function ConnectionModal({ instance, isOpen, onClose, onSuccess }: ConnectionModalProps) {
  ...
  }, [statusData?.status, isOpen, step, onSuccess, onClose])
