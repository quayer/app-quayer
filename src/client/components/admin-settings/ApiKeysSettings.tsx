...
export function ApiKeysSettings() {
  ...
  // Confirm action state (replaces native confirm())
  const [confirmAction, setConfirmAction] = useState<{ type: 'revoke' | 'delete'; keyId: string; keyName: string } | null>(null)
  ...
  const { data: apiKeys, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return ((result?.data?.data ?? result?.data) ?? []) as ApiKey[]
    },
  })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Confirm revoke/delete AlertDialog */}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmAction?.type === 'revoke'
                ? `Tem certeza que deseja revogar a API Key "${confirmAction?.keyName}"? Ela deixara de funcionar imediatamente.`
                : `Tem certeza que deseja deletar a API Key "${confirmAction?.keyName}" permanentemente? Esta acao nao pode ser desfeita.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
