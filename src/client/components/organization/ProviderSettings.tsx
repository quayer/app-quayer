...
export function ProviderSettings() {
    ...
    const handleValidateCredentials = async () => {
        ...
        try {
            // TODO: Conectar ao endpoint backend de validacao UAZapi quando disponivel
            // Por enquanto faz uma chamada HEAD para verificar se a URL responde
            const testResponse = await fetch(uazapiUrl, { method: 'HEAD', mode: 'no-cors' }).catch(() => null)
            if (testResponse !== null && uazapiKey.length > 5) {
                setValidationStatus('success')
                toast.success('Credenciais validadas (simulado) — validacao completa pendente no backend')
            } else {
