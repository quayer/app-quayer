...
export async function rollbackDeployment(
  ...
  // Reverse execution order (attach → instance → materialize_media →
  // materialize_team → materialize_pricing → publish). All three materialize
  // compensations are self-contained no-ops: ctx.state.{pricing,team,media} is NOT
  // reconstructed from the BuilderDeploymentRow, and the materialized
  // catalog/department/media are the user's source of truth (not deploy garbage),
  // so none is undone.
