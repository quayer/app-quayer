...
export interface RouletteCandidate {
  /** DepartmentMember.id */
  memberId: string
  /** User.id of the human agent */
  userId: string
  /** User.name (for return envelopes / notifications) */
  userName: string
 ...
 * position ASC, createdAt ASC) and the User.id of the last assigned member,
 * returns the first member strictly AFTER the cursor, wrapping around to the
 * start. If the cursor is null or no longer in the pool (member removed /
 ...
 * @param lastUserId    Department.lastAssignedUserId (cursor), or null
...
export function pickNextInOrder(
  ...
  const lastIdx = orderedPool.findIndex((c) => c.userId === lastUserId)
  if (lastIdx === -1) {
...
export async function loadActivePool(
  ...
  return members.map((m) => ({
    memberId: m.id,
    userId: m.userId,
    userName: m.user?.name ?? 'Atendente',
    position: m.position,
  }))
}
 ...
 *   3. Pick the next member after Department.lastAssignedUserId (circular).
 *   4. Advance the cursor: lastAssignedUserId = chosen.userId, lastAssignedAt = now().
 *
  ...
  try {
    return await runner(async (tx) => {
      ...
      const locked = await tx.$queryRaw<
        Array<{ id: string; lastAssignedUserId: string | null; isActive: boolean }>
      >`
        SELECT "id", "lastAssignedUserId", "isActive"
        FROM "Department"
      ...
      const chosen = pickNextInOrder(pool, dept.lastAssignedUserId)
      if (!chosen) {
      ...
      await tx.$executeRaw`
        UPDATE "Department"
        SET "lastAssignedUserId" = ${chosen.userId},
