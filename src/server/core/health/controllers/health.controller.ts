...
import { storeCircuitBreaker, externalApiCircuitBreaker } from '@/server/services/circuit-breaker'
import { adminProcedure } from '@/server/core/auth/procedures/auth.procedure'
      ...
      handler: async ({ response }) => {
        ...
        for (const instance of instances) {
          ...
          try {
            ...
            if (statusResponse.success && (statusResponse.data?.status as string) === 'connected') {
              results.push({
