...
export class OrganizationsRepository {
  ...
  /**
   * Count members with `master` role in organization
   */
  async countMasters(organizationId: string): Promise<number> {
    return database.userOrganization.count({
      where: {
        organizationId,
        isActive: true,
  ...
  async generateUniqueSlug(name: string): Promise<string> {
    ...
    let suffix = 1;
    while (true) {
      const existing = await database.organization.findUnique({ where: { slug } });
  ...
  async hasReachedInstanceLimit(organizationId: string): Promise<boolean> {
    ...
    const currentCount = await database.connection.count({
