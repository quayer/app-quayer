...
import { organizationsRepository } from '@/server/core/organizations/organizations.repository';
import { database } from '@/server/services/database';
import { authProcedure } from '@/server/core/auth/procedures/auth.procedure';
import { OrganizationRole, UserRole } from '@/lib/auth/roles';
...
export const invitationsController = igniter.controller({
  ...
  actions: {
    ...
    handler: async ({ request, response, context }) => {
      ...
      const body = request.body as ReturnType<typeof createInvitationSchema.parse>;
  ...
  acceptExisting: igniter.mutation({
    path: '/accept',
    method: 'POST',
    body: acceptInvitationExistingUserSchema,
    use: [authProcedure({ required: true })],
    handler: async ({ request, response, context }) => {
      const userId = context.auth?.session?.user?.id!;
      const body = request.body as ReturnType<typeof acceptInvitationExistingUserSchema.parse>;
      ...
      await organizationsRepository.addMember(invitation.organizationId, {
        userId,
        role: invitation.role as 'user' | 'master' | 'manager',
      });
  ...
  acceptNew: igniter.mutation({
    path: '/accept/new',
    method: 'POST',
    body: acceptInvitationNewUserSchema,
    handler: async ({ request, response }) => {
      const body = request.body as ReturnType<typeof acceptInvitationNewUserSchema.parse>;
      ...
      // Criar nova conta (sem senha — sistema passwordless)
      const newUser = await database.user.create({
        data: {
          email: invitation.email,
          name: body.name,
          emailVerified: new Date(), // Email verificado via convite
      ...
      // Auto-login: emitir tokens JWT para o novo usuário
  ...
  resend: igniter.mutation({
    ...
    handler: async ({ request, response, context }) => {
      const userId = context.auth?.session?.user?.id!;
      const { invitationId } = request.params as { invitationId: string };
      const body = request.body as ReturnType<typeof resendInvitationSchema.parse>;
