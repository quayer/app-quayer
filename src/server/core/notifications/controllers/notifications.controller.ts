...
const updatePreferencesSchema = z.object({
  ...
  pushDeployments: z.boolean().optional(),
})
  ...
  actions: {
    ...
    getPreferences: igniter.query({
      ...
      handler: async ({ context, response }) => {
        ...
        try {
          const prefs = await database.notificationPreferences.upsert({
            where: { userId: user.id },
            create: { userId: user.id },
            update: {},
          })
          return response.success({ data: prefs })
        } catch (error) {
          console.error('Error fetching notification preferences:', error)
          return response.badRequest('Erro ao obter preferências de notificação')
        }
      },
    }),

    // ==========================================
    // UPDATE NOTIFICATION PREFERENCES (current user)
    // ==========================================
    updatePreferences: igniter.mutation({
      ...
      handler: async ({ request, context, response }) => {
        ...
        try {
          ...
          return response.badRequest('Erro ao atualizar preferências de notificação')
        }
      },
    }),
