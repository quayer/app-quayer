...
import { apiKeyProcedure, authOrApiKeyProcedure } from '@/server/core/auth/procedures/api-key.procedure'
  ...
  actions: {
    ...
    list: igniter.query({
      ...
      handler: async ({ context, response }) => {
        ...
        const keys = await apiKeysRepository.listByOrganization(orgId)
        return response.json({ success: true, data: keys })
      },
    }),
    ...
    // ==========================================
    // VALIDATE API KEY (CLI / Programmatic Auth)
    // ==========================================
    validate: igniter.query({
      ...
      handler: async ({ request, context, response }) => {
        ...
        const organization = await context.db.organization.findUnique({
          where: { id: user.currentOrgId || user.organizationId },
          select: { name: true },
        })

        // Buscar dados da key pelo header id injetado pelo apiKeyProcedure
        const apiKeyId = request.headers.get('x-api-key-id')
        let apiKeyData = null
        if (apiKeyId) {
          apiKeyData = await context.db.apiKey.findUnique({
            where: { id: apiKeyId },
