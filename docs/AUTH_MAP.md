...
>
> **Onda D2 (G2 curadoria do catálogo visual, Jun/2026):** rotas de curadoria das imagens que a Onda D1 já extraiu/persistiu em `knowledge_images`, no mesmo `builderController` (guard `authOrApiKeyProcedure({ required: true })`, escopo por `currentOrgId`, `:id`/`:imageId` validados por UUID, ownership do projeto via `loadProject`). Sem migration nova. Curadoria org-scoped via `knowledgeImagesRepository` (todas as mutações usam `updateMany({ id, organizationId })` → 0 linhas afetadas ⇒ 404, sem vazar existência cross-tenant):
> - `GET /api/v1/builder/projects/:id/sources/images` — lista as imagens do projeto (todas as `KnowledgeSource` da collection `kb:${projectId}`, `deletedAt IS NULL`), org-scoped. Para CADA imagem ASSINA o `storageKey` on-read via `storage.getSignedUrl(BUCKETS.MEDIA, …)` → `imageUrl`. **FAIL-SAFE:** falha de assinatura de UMA imagem (ou storage indisponível) vira `imageUrl=null` só para aquela imagem — nunca derruba a lista. O `storageKey` NÃO é exposto ao FE (só a signed URL).
> - `PATCH /api/v1/builder/sources/images/:imageId` — curadoria de UMA imagem (exatamente UMA ação por chamada; Zod `{ caption? | deleted?:true | confirmed? }` com `refine` exigindo ≥1). Precedência `deleted > confirmed > caption`. Soft-delete só "deleta" (não des-deleta); `confirmed` alterna `confirmedAt = now()|null`.
> - `POST /api/v1/builder/projects/:id/sources/images/bulk` — ações em massa (Zod `{ action: 'approve_all' | 'delete_low_quality' }`): `approve_all` confirma todas as pendentes (`confirmedAt IS NULL`); `delete_low_quality` soft-deleta por heurística de baixa qualidade (sem caption OU dimensão `< LOW_QUALITY_MIN_DIMENSION_PX = 300px`). Retorna `{ confirmed }` ou `{ deleted }`.
> Estas rotas seguem as mesmas guards de auth do chat do Builder; não alteram o fluxo de login. Client tipado: `api.builder.listSourceImages` / `patchSourceImage` / `bulkSourceImages`.
>
> **Warm transfer B2 (pareamento WhatsApp dos atendentes, Jun/2026):** read-only no mesmo `builderController` (guard `authOrApiKeyProcedure({ required: true })`, escopo por `currentOrgId`):
> - `GET /api/v1/builder/connections/list` — lista as conexões WhatsApp (instâncias) da org ativa (id/name/phoneNumber/status) para o card `handoff_pairing` atribuir uma instância própria a cada atendente (warm transfer). Sem migration. Client tipado: `api.builder.listConnections`. (O pareamento em si reusa `POST /cards/handoff_pairing/submit` → grava `team.members[].connectionId`.)
>
> **Fase E / E4 (curadoria do catálogo de mídia enviável, Jun/2026):** rotas de LISTAGEM + curadoria do catálogo de mídia que o agente pode enviar no WhatsApp (foto/vídeo/PDF — **não** áudio), persistido no model de runtime `MediaAsset` (tabela `media_assets`). No mesmo `builderController` (guard `authOrApiKeyProcedure({ required: true })`, escopo por `currentOrgId`, `:id`/`:mediaId` validados por UUID, ownership do projeto via `loadProject`). Sem migration nova. Curadoria org-scoped via `mediaCurationRepository` (todas as mutações usam `updateMany({ id, organizationId })` → 0 linhas afetadas ⇒ 404, sem vazar existência cross-tenant). A rota de UPLOAD (`POST /api/v1/builder/media/upload`, multipart, E1) e a materialização (gallery + pricing, E2) já existiam; E4 acrescenta o list/patch que o FE da aba "Mídias" consome:
> - `GET /api/v1/builder/projects/:id/media` — lista os `MediaAsset` da collection do projeto (`deletedAt IS NULL`), org-scoped (filtra `organizationId` ALÉM do `collectionId` — defense-in-depth). Resolve a collection via `loadProject` + `resolveCollectionId`; projeto sem KB ⇒ `{ media: [] }` (no-op limpo, igual ao materialize). **Sign-on-read FAIL-SAFE por item** (espelha D2): se `externalUrl != null` usa-a **direto** (origem `pricing`, URL https externa, não assina); senão se `storageKey != null` ASSINA via `storage.getSignedUrl(BUCKETS.MEDIA, …)` dentro de try/catch → `url=null` só naquele item se a assinatura falhar — nunca derruba a lista; senão `url=null`. O `storageKey` NÃO é exposto ao FE (só `url`). Devolve `{ media: [{ id, url, mediaType, caption, category, source, mimeType, confirmedAt }] }`.
> - `PATCH /api/v1/builder/media/:mediaId` — curadoria de UMA mídia (exatamente UMA ação por chamada; Zod `{ caption? | deleted?:true | confirmed? }` com `refine` exigindo ≥1). Precedência `deleted > confirmed > caption`. Soft-delete só "deleta" (não des-deleta); `confirmed` alterna `confirmedAt = now()|null`; `caption` trima (vazio → `null`). Retorna `{ ok:true, mediaId, applied }`.
> Estas rotas seguem as mesmas guards de auth do chat do Builder; não alteram o fluxo de login. Client tipado: `api.builder.listProjectMedia` / `patchMediaAsset`.
