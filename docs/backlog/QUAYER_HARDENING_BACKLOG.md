...
- **Worker entrypoint** `scripts/start-workers.ts` — ativa os 3 workers (dev). Commit `e1fab02`. PROD: bundle esbuild (`scripts/build-workers.mjs` → `.next/standalone/workers/start-workers.js`) + serviço `worker` no compose (ver nota abaixo). Aguardando review de infra.
...
- **Auditoria (Wave 1):** 74 itens verificados — 49 done, 6 inert, **0 bugs**. Gaps remanescentes: ativação prod dos workers, normalização de encoding UTF-16→UTF-8.
- **QH-12 — verificado DONE (2026-06-06, corrige falso-positivo da auditoria):** o auditor P3 marcou "tokens em texto plano / `encryptToken` nunca chamado" — **incorreto.** `encryptToken` (AES-256-GCM) É chamado no OAuth callback (`app/api/v1/calendar/oauth/callback/route.ts:260`) e o refresh_token vive cifrado em `OrganizationProvider`; `decryptToken` faz passthrough de legado (migração lazy zero-downtime); apiKeys BYOK cifradas em `providers.repository.ts`. Nenhum write de `credentials` sem cifra. Test: `calendar-crypto.service.test.ts`.
...
- **Limitação conhecida (QH-02):** ~~ao estourar o rate limit de instância, a resposta retorna `rateLimited=true` sem reenfileiramento~~ — **resolvido (2026-06-05).** Ao estourar o limite de **instância**, `sendAgentResponse` agenda um retry com `delay=retryAfterMs` (`outbound-retry.queue.ts`, fila BullMQ `quayer:outbound-retry`, piso 1s/teto 60s), com contador `attempt` e cap `MAX_RETRY_ATTEMPTS=5`; ao esgotar (ou sem scheduler injetado, ou falha ao agendar) cai na dead-letter existente (`outbound:deadletter`). Só o escopo **instância** é retentado — os limites de **contato/org** são throttles de produto deliberados (drop-by-design). Worker registrado em `registerAllWorkers`.
  - **Ativação dos workers (parcial — 2026-06-06):** criado o entrypoint dedicado `scripts/start-workers.ts` (+ `npm run start:workers`) que chama `registerAllWorkers` e agenda o cron do session-close, com graceful shutdown. Sobe os 3 workers (`outbound-retry`, `source-enrich`, `session-close`) em qualquer ambiente com o source + `tsx` (dev/local/homol-com-source).
    - **Ativação de PROD (2026-06-06 — opção (a), aguardando review de infra):** a imagem de produção é um **Next standalone** (Dockerfile runner copia só `.next/standalone`, sem `src/` nem `tsx`), então `tsx scripts/start-workers.ts` **não roda na imagem prod**. Resolvido bundlando o entrypoint com **esbuild** (`scripts/build-workers.mjs` + `npm run build:workers`): empacota `start-workers.ts` + todo o caminho de envio num único CJS em `.next/standalone/workers/start-workers.js`, resolvendo aliases `@/*` em build-time e **externalizando** `@prisma/client`/`@prisma/adapter-pg`/`bullmq`/`ioredis`/`redis`/`pg`/`@igniter-js/*`/`@sentry/node` (já presentes no node_modules do runner). O **Dockerfile** roda `build:workers` no builder (depois do `next build`) e o runner copia `workers/`; o **compose.homol.yml** ganhou o serviço `worker` (mesma imagem `quayer-app:homol`, `command: node workers/start-workers.js`, env/networks/depends_on espelhando o `app`, healthcheck por processo). Bundle validado: 2.1 MB, `node --check` ok, externals como `require()`, zero `@/` não-resolvido. Em dev segue valendo `OUTBOUND_RETRY_SYNC=1` p/ processar inline sem worker.
