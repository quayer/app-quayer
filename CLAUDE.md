...
| `BuilderJourneyEvent` (funil da jornada v2 — eventos repetíveis de progresso do usuário no Builder; **sem FK** como `BuilderToolCall.messageId`; `projectId` + `organizationId` por tenant, `journeyVersion` 1\|2 congelado no evento, `event` em vocabulário fechado TS (`journey-events.ts`), `metadata` Json tipado sem PII (NFR-02/LGPD); índices `(organizationId,event,createdAt)` p/ funil e `(projectId,createdAt)` p/ timeline) | ai-module/builder | `builder_journey_events` |
...
| `MediaAsset` (Fase E — catálogo de mídia ENVIÁVEL pelo agente: foto/vídeo/PDF, NÃO áudio; FK→KnowledgeCollection, `collectionId = ragCollectionId`; `source ∈ upload/gallery/pricing`, `@@unique(source,sourceRef)`; `storageKey?` assina on-read OU `externalUrl?` direto; runtime só envia `confirmedAt IS NOT NULL AND deletedAt IS NULL`. Materializado por `materialize_media` na saga; upload via `POST /api/v1/builder/media/upload`; retrieval via tool `buscar_media` que NUNCA envia — o outbound envia via tag) | ai-module (catálogo de mídia) | `media_assets` |
| `CustomIntegration` (Integration Builder W1 — 1:1 com `AgentTool` via `agentToolId String? @unique` FK **SetNull**; DELETE = soft-delete `deletedAt` na CustomIntegration + **hard-delete do AgentTool**, o que libera o nome `@@unique([organizationId, name])` para reuso e a linha auditada sobrevive com `agentToolId = null`; `status='active'` ⇄ `AgentTool.isActive=true` — paridade playground↔produção via `getCustomTools`; `credentials Json?` = valores **cifrados individualmente** (`lib/crypto`, NFR-01), **NUNCA** no `builderState`; FKs org/builderProject Cascade), `IntegrationTestCall` (auditoria de teste — outcome/httpStatus/durationMs, **sem payloads**; Cascade no delete da integração) | ai-module/builder/integrations | `custom_integrations`, `integration_test_calls` |
...
| `CalendarConnection`, `Department`, `DepartmentMember` (M1: `DepartmentMember.userId` NULLABLE/SetNull + `name` + `whatsapp` — membro pode ser "nome + WhatsApp"; roleta re-chaveada por `memberId` envia notificação WhatsApp 6A com fallback in-app). **`AIAgentConfig.departmentId String?`** = vínculo ESTRUTURADO agente↔departamento: `materialize_team` grava o `Department.id` da roleta (fonte autoritativa do id) e o `dispatch_to_agent` lê via `ctx.agentDepartmentId` como FALLBACK quando o LLM omite o departmentId (robusto a sombreamento do bloco ROLETA no systemPrompt por AgentPromptVersion ACTIVE). **`Department.warmTransferOpeningMessage String?`** (B1b) = mensagem de abertura EDITÁVEL do warm transfer: `builderState.team.openingMessage` → `materialize_team` (clear-on-empty); `dispatch_to_agent` lê fail-open; `warm-transfer` interpola `{nome}` antes de enviar pela conexão do membro; NULL = texto default. | builder/communication (calendário + roleta) | `calendar_connections`, `Department` (PascalCase, sem @@map), `department_members` |
...
| `ScheduledAutomation` (regra de automação **proativa** — épico `specs/builder-proatividade/`, RUNTIME; `trigger` lead_idle/appointment_before/after/birthday/renewal_due/custom_date; **sem FK** como BuilderJourneyEvent; org/project; materializada da F1 `builderState.proactive`), `ScheduledMessage` (INSTÂNCIA de envio por contato — também ad-hoc via `create_followup`; índice `(status,scheduledAt)` p/ o worker BullMQ; cancel-on-inbound por `(organizationId,contactPhone)`; `automationId` lógico, null=ad-hoc), `ContactOptOut` (opt-out por `@@unique(organizationId,phone)` — **não há modelo Contact**). ⚠️ Migration `20260613000000_proactive_scheduling` gerada no client; **DB apply pendente** (sem DATABASE_URL local) — roda via `node prisma/migrate.js` no test DB/deploy. F2 runtime ainda NÃO implementado. | ai-module (proatividade/runtime) | `scheduled_automations`, `scheduled_messages`, `contact_opt_outs` |
| `OutboundDispatch` (FSM outbound durável — **pré-requisito do F2b proatividade**; checkpoint POR BLOCO do `providerMessageId` em `sendAgentResponse` p/ não reenviar blocos duplicados em crash no meio do loop; `dispatchKey` = `sha256(sessionId:inboundMessageId)` `@unique` → idempotência; `status` queued/sending/sent/partial/failed + `blocks Json` [{idx,providerMessageId?,status}]; **FAIL-OPEN** — ausência de dispatchKey/dep ou erro de dispatch cai p/ o envio de HOJE, nunca bloqueia a mensagem; **sem FK** como scheduled_automations/builder_journey_events; org-scoped; índice `(status,updatedAt)`). ⚠️ Migration `20260613010000_outbound_dispatch` gerada no client; **DB apply pendente** (sem DATABASE_URL local) — via `node prisma/migrate.js` no test DB/deploy. | communication (outbound/runtime) | `outbound_dispatches` |
